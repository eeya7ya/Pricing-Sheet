...
function getTransporter() {
  ...
  if (!user || !pass) {
    console.error("[email] Missing env vars: GMAIL_USER or GMAIL_APP_PASSWORD not set.");
    return null;
  }
...
export async function sendAccountRequestNotification(opts: {
  ...
  if (!transporter || !adminEmail) {
    console.error("[email] Cannot send: transporter null or ADMIN_EMAIL missing. adminEmail=", adminEmail);
    return;
  }
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Smart Pricing Sheet" <${gmailUser}>`,
      to: adminEmail,
      replyTo: email,
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;border:1px solid #e5e7eb;border-radius:12px;">
          ...
          <p style="margin:0;font-size:12px;color:#9ca3af;">Log in to the Admin panel to approve this request.</p>
        </div>
      `,
    });
...
export async function sendCredentialsEmail(opts: {
  ...
  try {
    ...
    console.log("[email] Credentials sent, messageId:", info.messageId);
  } catch (err) {
    console.error("[email] sendCredentials failed:", err);
    throw err;
  }
}
